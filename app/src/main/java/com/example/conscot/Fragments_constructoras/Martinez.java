...
public class Martinez extends Fragment {
    ...
    private ArrayList<productos_constructoras> obtenerDatos() {
        ...
        try {
            switch (Construccion){
                case "Piso":
                    rs = new Conexion().Consulta("Select Productos.Producto, Productos.Precio from Productos\n" +
                            ...
                            "where  (tipo.tipo='Cemento' Or  Productos.Producto='grava 1 m3' or Productos.Producto='Arena 1 m3') and Productos.id_Constructora=2;");
                    break;
                case "Losa":
                    rs = new Conexion().Consulta("Select Productos.Producto, Productos.Precio from Productos\n" +
                            "inner join tipo on Productos.id_tipo = tipo.id_tipo \n" +
                            "where  (tipo.tipo='Cemento' Or  Productos.Producto='grava 1 m3'or Productos.id_sub_tipo=2 or Productos.Producto='arena 1 m3') and Productos.id_Constructora=2;");
            ...
            while (rs.next()) {
                lista.add(new productos_constructoras("" + rs.getString("Producto"), "" + rs.getString("Precio"),"Martinez"));
            }
        } catch (Exception e) {
